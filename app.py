...
def load_all():
    ...
    # cb_model.pkl goc nang 1.1 GB (sim_matrix float64)
    # -> thay bang tfidf_matrix.npz (0.1 MB) + rebuild khi khoi dong (~2s)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Dữ liệu: Olist Brazil 2017–2018")
...
if "Tổng quan" in page:
    ...
    st.caption("Dữ liệu 2017–2018 · Olist E-commerce Brazil")
    ...
    with col_chart:
        ...
        st.caption("🟡 Tháng 11/2017: Black Friday — cao nhất năm · 🟢 2018: doanh thu ổn định cao hơn")
...
# ══════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        # ── Bộ lọc: Phân khúc + Dropdown mã KH + Số gợi ý ──────────────
        col_seg, col_uid, col_topn = st.columns([2, 4, 1])

        with col_seg:
            seg_filter = st.selectbox(
                "Phân khúc",
                options=["Tất cả", "one_time", "occasional", "regular"],
                format_func=lambda x: "Tất cả phân khúc" if x == "Tất cả"
                            else SEG_LABELS.get(x, x),
                key="tab1_seg_filter",
            )

        with col_uid:
            # Lọc danh sách user theo segment đã chọn
            if seg_filter == "Tất cả":
                uid_pool = sorted(seg_dict.keys())
            else:
                uid_pool = sorted([u for u, s in seg_dict.items()
                                   if s == seg_filter])

            # Tạo nhãn hiển thị: "mã... (Phân khúc · Khu vực)"
            @st.cache_data(show_spinner=False)
            def build_uid_labels(_train_df, _seg_dict, _pool):
                info = _train_df.groupby("customer_unique_id").agg(
                ...
                labels = {}
                for uid in _pool:
                    seg  = _seg_dict.get(uid, "one_time")
                    meta = info.get(uid, {})
                    city = meta.get("city", "?")
                    st_  = meta.get("state", "?")
                    n    = meta.get("n", 0)
                    labels[uid] = (
                        f"{uid[:24]}...  "
                        f"({SEG_LABELS.get(seg, seg)} · {city}/{st_} · {n} đơn)"
                    )
                return labels

            uid_labels = build_uid_labels(train_df, seg_dict, tuple(uid_pool))

            user_id = st.selectbox(
                "Chọn mã khách hàng",
                options=uid_pool,
                format_func=lambda x: uid_labels.get(x, x),
                key="tab1_uid_select",
            )
